...
clean:
	docker-compose -p csproject_notifications down --rmi local --volumes
...
setup-local: check_network
	docker-compose -p csproject_notifications down --rmi local --volumes
	docker-compose -f docker-compose-local.yml -p csproject_notifications up -d --build
	docker exec -it notifications_api composer install

setup-dev: check_network
	docker-compose -p csproject_notifications down --rmi local --volumes
	docker-compose -f docker-compose-dev.yml -p csproject_notifications up -d --build
	docker exec -it notifications_api composer install
